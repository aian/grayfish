...
<xsl:template match="d:entry|d:entrytbl" name="entry">
  ...
  <xsl:variable name="rowsep">
    <xsl:choose>
      ...
      <xsl:when test="not(ancestor::d:thead) and @morerows and not(@morerows &lt;                  ( count(ancestor-or-self::d:row[1]/following-sibling::d:row) +                    count(ancestor::d:tgroup/d:tfoot/d:row)) )">
        <xsl:value-of select="0"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:call-template name="inherited.table.attribute">
