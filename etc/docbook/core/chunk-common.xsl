...
<xsl:template name="navigation">
  ...
  <xsl:param name="nav.location" select="''"/>
  ...
  <xsl:if test="$suppress.navigation = '0' and $suppress.header.navigation = '0'">
    <div>
      <xsl:attribute name="class">
        <xsl:value-of select="concat('nav', $nav.location)"/>
      </xsl:attribute>
      <xsl:if test="$row2">
          ...
          <xsl:if test="$row2">
            <tr>
              ...
              <td width="60%" align="center">
                <xsl:choose>
                  <xsl:when test="generate-id(.) != generate-id($home)">
                    <a accesskey="h">
                      <xsl:attribute name="href">
                        <xsl:call-template name="href.target">
                          <xsl:with-param name="object" select="$home"/>
        ...
        <div id="content">
          <div id="main">
            <xsl:call-template name="navigation">
              ...
              <xsl:with-param name="next" select="$next"/>
              <xsl:with-param name="nav.context" select="$nav.context"/>
              <xsl:with-param name="nav.location" select="'header'"/>
            </xsl:call-template>
            ...
            <xsl:call-template name="navigation">
              ...
              <xsl:with-param name="next" select="$next"/>
              <xsl:with-param name="nav.context" select="$nav.context"/>
              <xsl:with-param name="nav.location" select="'footer'"/>
            </xsl:call-template>
