  ...
  <xsl:choose>
    ...
    <xsl:otherwise>
      <div>
        ...
        <xsl:choose>
          <xsl:when test="$role='with-coverage' and biblioid[@class='isbn'] != ''">
            <div class="bibliocoverage">
              <img>
                <xsl:attribute name="src">
                  <xsl:text>http://images.amazon.com/images/P/</xsl:text>
                  <xsl:value-of select="biblioid"/>
                  <xsl:text>.09.MZZZZZZZ.jpg</xsl:text>
                </xsl:attribute>
              </img>
            </div>
            <div class="bibliospec"><!-- qune original -->
...
<xsl:template match="address" mode="bibliography.mode">
  <xsl:param name="separator" select="$biblioentry.item.separator"/>
  <span>
    <xsl:call-template name="common.html.attributes"/>
    <xsl:call-template name="id.attribute"/>
    <xsl:apply-templates mode="bibliography.mode"/>
...
<xsl:template match="title" mode="bibliography.mode">
  ...
  <span>
    <xsl:call-template name="common.html.attributes"/>
    <xsl:call-template name="id.attribute"/>
    <xsl:call-template name="biblioentry.title"/>
    <xsl:copy-of select="$separator"/>
  </span>
</xsl:template>
...
<xsl:template match="bibliocoverage|bibliorelation|bibliosource" mode="bibliography.mode">
  <xsl:param name="separator" select="$biblioentry.item.separator"/>
  <span>
    <xsl:call-template name="common.html.attributes"/>
    <xsl:call-template name="id.attribute"/>
