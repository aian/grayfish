...
<xsl:template match="simplelist[@role='extern']">
  <dl class="extern">
    <xsl:apply-templates mode="simplelist.extern"/>
  </dl>
</xsl:template>

<xsl:template match="member" mode="simplelist.extern">
  <xsl:param name="path" select="@xl:href"/>

  <xsl:variable name="href">
    <xsl:call-template name="replace-all-string">
      <xsl:with-param name="text"    select="$path"/>
      <xsl:with-param name="replace" select="'.dbk'"/>
      <xsl:with-param name="by"      select="'.html'"/>
    </xsl:call-template>
  </xsl:variable>

  <xsl:variable name="doc">
    <xsl:apply-templates select="document($path)" mode="stripNS"/>
  </xsl:variable>

  <xsl:variable name="title"    select="exsl:node-set($doc)/*/*[1]/title"/>
  <xsl:variable name="date"     select="exsl:node-set($doc)/*/*[1]/date"/>
  <xsl:variable name="pubdate"  select="exsl:node-set($doc)/*/*[1]/pubdate"/>
  <xsl:variable name="abstract" select="exsl:node-set($doc)/*/*[1]/abstract"/>

  <xsl:if test="$title">
    <dt class="entry">
      <xsl:choose>
        <xsl:when test="$pubdate">
          <xsl:apply-templates select="$pubdate" mode="simplelist.extern"/>
        </xsl:when>
        <xsl:when test="$date">
          <xsl:apply-templates select="$date" mode="simplelist.extern"/>
        </xsl:when>
      </xsl:choose>
      <span class="title">
        <a>
          ...
          -->
          <xsl:apply-templates select="$title" mode="simplelist.extern"/>
        </a>
      </span>
    </dt>
    <dd>
      <xsl:if test="$abstract">
        <xsl:apply-templates select="$abstract" mode="simplelist.extern"/>
      </xsl:if>
    </dd>
  </xsl:if>
</xsl:template>

<xsl:template match="title" mode="simplelist.extern">
  <xsl:apply-templates mode="titlepage.mode"/>
</xsl:template>

<xsl:template match="date" mode="simplelist.extern">
  <span class="date">
    <xsl:apply-templates mode="titlepage.mode"/>
  </span>
</xsl:template>

<xsl:template match="pubdate" mode="simplelist.extern">
  <span class="date">
    <xsl:apply-templates mode="titlepage.mode"/>
  </span>
</xsl:template>

<xsl:template match="abstract" mode="simplelist.extern">
  <div class="abstract">
    <xsl:apply-templates mode="titlepage.mode"/>
  </div>
</xsl:template>

<xsl:template match="text()" mode="article-list.mode"/>
